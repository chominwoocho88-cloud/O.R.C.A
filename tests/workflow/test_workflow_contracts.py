...
def _repo_root() -> Path:
    for path in Path(__file__).resolve().parents:
        if (path / ".github" / "workflows").is_dir():
            return path
    raise RuntimeError("Repository root not found from workflow contract test")
...
class TestHighRiskWorkflowStatePersistenceContracts(unittest.TestCase):
    ...
    def test_jackal_session_keeps_replay_push_safety_logs(self):
        ...
        for marker in (
            ...
            "stage_jackal_path()",
            "Skipping missing optional JACKAL path:",
            "Staged JACKAL paths:",
            "Staged state diff:",
            "git diff --cached --name-status",
            "no state changes to commit",
...
class TestWorkflowDispatchUiContracts(unittest.TestCase):
    ...
    def test_wave_f_three_year_dispatch_defaults_are_current(self):
        ...
        self.assertIn('default: "758"', backfill)
        self.assertIn('default: "3874"', backfill)
        self.assertIn("EXPECTED_SNAPSHOTS_VALUE=\"758\"", backfill)
        self.assertIn("EXPECTED_LINKED_LESSONS_VALUE=\"3874\"", backfill)
        self.assertIn("EXPECTED_SNAPSHOTS', '758'", backfill)
        self.assertIn("EXPECTED_LINKED_LESSONS', '3874'", backfill)

        self.assertIn('default: "758"', clustering)
        self.assertIn('default: "3874"', clustering)
        self.assertIn("EXPECTED_SNAPSHOTS_VALUE=\"758\"", clustering)
        self.assertIn("EXPECTED_LINKED_LESSONS_VALUE=\"3874\"", clustering)
        self.assertIn("EXPECTED_SNAPSHOTS', '758'", clustering)
        self.assertIn("EXPECTED_LINKED_LESSONS', '3874'", clustering)
        ...
        self.assertIn('default: "3874"', archive)
...
class TestBacktestWorkflowContracts(unittest.TestCase):
    def test_orca_backtest_uses_3year_defaults(self):
        ...
        self.assertIn('EXPECTED_MIN_CANDIDATES_VALUE="3874"', text)
        self.assertIn('EXPECTED_MIN_LESSONS_VALUE="3874"', text)
        self.assertIn('EXPECTED_MIN_CANDIDATES", "3874"', text)
        self.assertIn('EXPECTED_MIN_LESSONS", "3874"', text)
