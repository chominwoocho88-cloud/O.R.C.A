...
Examples:
  ...
  python -m orca.reset --jackal
  python -m orca.reset --orca --jackal
  python -m orca.reset --orca --dry-run
"""
import argparse
import shutil
...
ORCA_ACCURACY_DEFAULT = {
    ...
    "_reset_at": "",
    "_note": "orca_reset.py executed",
}

ORCA_WEIGHTS_DEFAULT = {
    "version": 1,
    "last_updated": "",
...
def reset_orca(dry: bool = False) -> None:
    print("\n🔄 " + ORCA_NAME + " 데이터 초기화")
    DATA_DIR.mkdir(exist_ok=True)

    _write(DATA_DIR / "accuracy.json", ORCA_ACCURACY_DEFAULT.copy(), dry)
    _write(LESSONS_FILE, ORCA_LESSONS_DEFAULT.copy(), dry)
    _write(WEIGHTS_FILE, ORCA_WEIGHTS_DEFAULT.copy(), dry)
    _write(DATA_DIR / "sentiment.json", ORCA_SENTIMENT_DEFAULT.copy(), dry)
    _write(DATA_DIR / "rotation.json", ORCA_ROTATION_DEFAULT.copy(), dry)
...
def main():
    parser = argparse.ArgumentParser(description=ORCA_NAME + "/" + JACKAL_NAME + " 학습 데이터 초기화")
    parser.add_argument("--orca",    action="store_true", help=ORCA_NAME + " 데이터 초기화")
    parser.add_argument("--jackal",  action="store_true", help=JACKAL_NAME + " 데이터 초기화")
    parser.add_argument("--dry-run", action="store_true", help="실제 파일 수정 없이 미리보기")
    args = parser.parse_args()

    if not args.orca and not args.jackal:
        parser.print_help()
        print("\n⚠️  --orca 또는 --jackal 중 하나 이상 지정하세요.")
        return
    ...
    if not dry:
        print("   다음 단계: ORCA backtest 실행 권장 (python -m orca.backtest --months 6)")
