...
HUNT_LOG_FILE = _REPO_ROOT / "jackal" / "hunt_log.json"
JACKAL_WEIGHTS_FILE = _REPO_ROOT / "jackal" / "jackal_weights.json"
    ...
    def jackal_block():
        ...
        if not hunt_log:
            return (
                ...
                f'🦊 {JACKAL_NAME} 미실행 — hunt_log.json 없음</div>',
                "", ""
            )
...
{"" if not hunt_log else f'''<div class="sec">
  <div class="sh"><span class="st">🦊 {JACKAL_NAME} 스윙 타점</span><span class="sn">최근 알림 · 100→5단계</span></div>
...
<div class="footer">
  분석일 {_e(adt)} · {ORCA_NAME} · {_e(ORCA_FULL_NAME)}<br>
...
if(rEl)new Chart(rEl,{{type:'bar',
  ...
  options:{{indexAxis:'y',responsive:true,maintainAspectRatio:false,
    ...
    print("dashboard generated: " + str(OUTPUT_FILE))
    return html
