...
- 추가 504 trading days를 materialize해서 총 756 trading days, 약 3780 lessons로 확장합니다.
...
| Wave F Backfill | `expected_snapshots` | `252` | `756` |
| Wave F Backfill | `expected_linked_lessons` | `1260` | `3780` |
...
- `dry_run=false`
- `skip_existing=true`
- `expected_snapshots=756`
- `expected_linked_lessons=3780`
...
- `expected_snapshots=756`
- `expected_linked_lessons=3780`
- `min_silhouette=0.10`부터 시작 후 결과 확인
...
python scripts/build_lesson_clusters.py \
  ...
  --append \
  --n-clusters 12 \
  --expected-snapshots 756 \
  --expected-linked-lessons 3780 \
  --min-silhouette 0.10 \
  --verbose
```
...
- `candidate_lessons`: 약 3780
- `candidate_registry(backtest)`: 약 3780
- `lesson_context_snapshot(backtest_backfill)`: 약 756
- latest clustering run mappings: 약 756
- latest archive run rows: 약 3780
...
3. Wave F Backfill workflow: `expected_snapshots=756`, `expected_linked_lessons=3780`
4. Wave F Clustering workflow: `append_mode=true`, `n_clusters=12`부터 dry-run
5. Wave F Archive workflow: `append_mode=true`, `expected_archive_count=3780`
