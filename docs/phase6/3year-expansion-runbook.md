...
- 추가 506 trading days를 materialize해서 총 758 trading days, 약 3874 lessons로 확장합니다.
...
| Wave F Backfill | `expected_snapshots` | `252` | `758` |
| Wave F Backfill | `expected_linked_lessons` | `1260` | `3874` |
...
- `dry_run=false`
- `skip_existing=true`
- `expected_snapshots=758`
- `expected_linked_lessons=3874`
...
python scripts/build_lesson_clusters.py \
  ...
  --append \
  --n-clusters 8 \
  --expected-snapshots 758 \
  --expected-linked-lessons 3874 \
  --min-silhouette 0.11 \
  --verbose
```
...
- `candidate_lessons`: 약 3874
- `candidate_registry(backtest)`: 약 3874
- `lesson_context_snapshot(backtest_backfill)`: 약 758
- latest clustering run mappings: 약 758
- latest archive run rows: 3874
...
3. Wave F Backfill workflow: `expected_snapshots=758`, `expected_linked_lessons=3874`, `confirm_apply=APPLY_WAVE_F`
4. Wave F Clustering workflow: `append_mode=true`, `n_clusters=8`, `expected_snapshots=758`, `expected_linked_lessons=3874`, `confirm_apply=APPLY_WAVE_F`
5. Wave F Archive workflow: `append_mode=true`, `expected_archive_count=3874`, `confirm_apply=APPLY_WAVE_F`
