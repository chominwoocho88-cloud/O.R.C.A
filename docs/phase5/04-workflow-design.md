...
현재 workflow 관찰:
- `orca_daily.yml`, `jackal_tracker.yml`, `jackal_session.yml` 는 `orca_state.db` 에 대해 explicit `wal_checkpoint(TRUNCATE)` 를 호출한다
- `jackal_session.yml` 는 explicit checkpoint block 이 없다
...
- `orca_daily.yml`, `jackal_tracker.yml`, `jackal_session.yml` 의 existing ORCA checkpoint pattern 과 모양이 맞는다

단점:
- Python 코드 추가가 필요하다
- Phase 5-D-c 직전이 아니라 Phase 5-C 범위를 다시 건드리는 성격이 된다
- `jackal_session.yml` 에도 checkpoint invocation 을 넣어야 일관된다
...
- 반대로 `jackal_session.yml` 의 hotfix는 backup/restore 중심이라 sidecar whitelist 로 처리하는 `Option 1` 과도 잘 맞는다
...
현재 대응 차이:
- `jackal_session.yml`: strongest pattern
  - `fetch + reset + clean + restore + 3-retry`
- `jackal_tracker.yml`: mid-level pattern
  - `stash + fetch + pull --rebase + stash pop`
- `jackal_session.yml` / `orca_daily.yml`: simple `push || pull --rebase || rebase --abort; push`
...
3. `jackal_session.yml` hotfix 에 `jackal_state.db` 보호 포함
4. YAML syntax 검증 통과
...
- `orca_daily.yml`, `jackal_tracker.yml`, `jackal_session.yml` 에서 sidecar 를 모두 `git add -f` 할지
- `jackal_session.yml` 에서는 whitelist / backup / restore 까지 sidecar 를 확장할지

### Q3. `jackal_session.yml` hotfix backup/restore 확장 범위
- 본체만
- 본체 + sidecar
- 별도 DB array 분리 여부
