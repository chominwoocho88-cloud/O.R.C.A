...
| `wave_f_archive.yml` | ORCA DB/archive DB commit | Dispatch dry-run with `dry_run=true`, `force_rebuild=false`, `append_mode=false`, `expected_archive_count=3874`, `confirm_apply` empty. Approved non-dry append runs should set `append_mode=true` and `confirm_apply=APPLY_WAVE_F`. |
...
- Current 3-year Wave F inventory baseline is `758` backfill snapshots, `3874` linked backtest lessons, `758` clustered snapshots, and `3874` archived lessons. The latest approved append run produced `cluster_run_20260430054416_7a7547cb` and `archive_run_20260430063226_944c2f2c`. The current 8-cluster silhouette is about `0.1114`, so the action smoke threshold remains `0.11`.
