...
from jackal.historical_context import apply_historical_adjustment as _apply_historical_context
from jackal.historical_context import historical_alert_lines as _historical_alert_lines
from jackal.historical_context import market_features_from_aria as _historical_market_features_from_aria
from jackal.historical_context import try_retrieve_historical_context as _try_retrieve_historical_context
from jackal.json_parse import safe_parse_json as _parse_llm_json
from jackal.probability import apply_probability_adjustment, load_probability_summary
...
{{"devil_score": 0~100 (높을수록 회의적),
  ...
  "structural_decline": true/false (구조적 하락인가? 단순 과매도는 false),
  "is_dead_cat": true/false (구조적 하락 추세 + 약한 반등 신호만 존재 + 재하락 가능성이 모두 명확할 때만 true. 단순 RSI 과매도/일반 반대/보통 위험은 false),
  "thesis_killer_hit": true/false (Analyst 핵심 매수 thesis가 진짜 깨졌을 때만 true. 단순 반대/위험 강조는 false; thesis 무효화 증거 필요),
