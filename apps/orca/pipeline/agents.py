"""
orca_agents.py — ORCA 4-Agent Pipeline
...
MODEL_HUNTER        = get_orca_env("ORCA_MODEL_HUNTER", _DEFAULT_HAIKU)
MODEL_ANALYST       = get_orca_env("ORCA_MODEL", _DEFAULT_SONNET)
MODEL_DEVIL         = get_orca_env("ORCA_MODEL", _DEFAULT_SONNET)
MODEL_REPORTER_FULL = get_orca_env("ORCA_MODEL", _DEFAULT_SONNET)
MODEL_REPORTER_LITE = get_orca_env("ORCA_MODEL_LITE", _DEFAULT_SONNET)
...
def agent_hunter(date_str: str, mode: str, market_data: dict = None, memory: list = None) -> dict:
    ...
    if market_data:
        try:
            from orca.data import format_for_hunter
...
def agent_analyst(hunter_data: dict, mode: str, lessons_prompt: str = "", memory: list = None) -> dict:
    ...
    try:
        from apps.orca.analysis import build_lessons_prompt as _blp
    ...
    if memory and len(memory) >= 3:
        try:
            from apps.orca.analysis import get_pattern_context
...
def agent_reporter(hunter: dict, analyst: dict, devil: dict,
    ...
    if memory:
        try:
            from apps.orca.analysis import build_compact_history
    ...
    data_missing = []
    try:
        from orca.data import load_market_data
        md = load_market_data()
        ...
        # 외국인 수급: KIS 실데이터 > EWY 프록시 > 없음
        if md.get("krx_flow_source") == "kis":
            pass  # KIS 실데이터 연결됨 — 감점 없음
        elif md.get("ewy","N/A") != "N/A":
            data_missing.append("KIS수급(EWY프록시대체)"); data_quality_score -= 3
        else:
