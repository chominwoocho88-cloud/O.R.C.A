...
실행:
  python jackal/jackal_tracker.py            # 일반 실행
  python jackal/jackal_tracker.py --dry-run  # 저장 없이 결과 미리보기
  python jackal/jackal_tracker.py --all      # alerted 여부 무관 전체 처리
"""
...
def run_tracker(dry_run: bool = False, all_entries: bool = False) -> dict:
    ...
    try:
        logs: list = json.loads(HUNT_LOG_FILE.read_text(encoding="utf-8"))
    except Exception as e:
